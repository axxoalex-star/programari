...
<div class="page-header">
    ...
    <p>Modifică detaliile programării</p>
</div>

<form method="POST" action="{{ route('admin.appointments.update', $appointment) }}" style="max-width: 800px;">
    @csrf
    @method('PUT')
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
        ...
        <div class="form-group">
            ...
            <input type="tel" id="client_phone" name="client_phone" value="{{ old('client_phone', $appointment->client_phone) }}" required>
            @error('client_phone')
                <span style="color: #dc3545; font-size: 13px;">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px;">
        ...
        <div class="form-group">
            ...
            <select id="status" name="status" required>
                <option value="confirmed" {{ old('status', $appointment->status == 'pending' ? 'confirmed' : $appointment->status) == 'confirmed' ? 'selected' : '' }}>Confirmat</option>
                <option value="cancelled" {{ old('status', $appointment->status) == 'cancelled' ? 'selected' : '' }}>Anulat</option>
                <option value="completed" {{ old('status', $appointment->status) == 'completed' ? 'selected' : '' }}>Finalizat</option>
            </select>
            @error('status')
                <span style="color: #dc3545; font-size: 13px;">{{ $message }}</span>
            @enderror
...
<script>
    // Încarcă doctorii când se schimbă departamentul
    document.getElementById('department_id').addEventListener('change', async function() {
        ...
        try {
            ...
            let options = '<option value="">Selectează doctor</option>';
