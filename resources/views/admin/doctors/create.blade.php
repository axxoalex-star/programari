...
@section('title', 'Adaugă Doctor')

@section('content')
<div class="page-header">
    <h1>Adaugă Doctor Nou</h1>
    <p>Completează formularul pentru a adăuga un doctor</p>
</div>

<form method="POST" action="{{ route('admin.doctors.store') }}" style="max-width: 800px;">
    @csrf
        ...
        <div style="display:grid; grid-template-columns: repeat(2, minmax(200px, 1fr)); gap:8px;">
            @foreach($departments as $department)
                <label class="form-check" style="gap:8px;">
                    <input type="checkbox" name="department_ids[]" value="{{ $department->id }}" {{ collect(old('department_ids', []))->contains($department->id) ? 'checked' : '' }}>
    ...
    <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 15px;">
        <div class="form-group">
            ...
            <input type="text" id="title" name="title" value="{{ old('title', 'Dr.') }}" required placeholder="Dr., Prof. Dr.">
            @error('title')
                <span style="color: #dc3545; font-size: 13px;">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
        ...
        <div class="form-group">
            ...
            <input type="tel" id="phone" name="phone" value="{{ old('phone') }}" required placeholder="0740123456">
            @error('phone')
                <span style="color: #dc3545; font-size: 13px;">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
        <div class="form-group">
            ...
            <input type="number" id="consultation_duration" name="consultation_duration" value="{{ old('consultation_duration', 30) }}" min="15" max="180" required>
        ...
        <div class="form-group">
            ...
            <input type="number" id="consultation_price" name="consultation_price" value="{{ old('consultation_price', 200) }}" min="0" step="0.01" required>
    ...
    <div style="border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; margin-bottom: 15px; background: #f8f9fa;">
        <div style="display: grid; grid-template-columns: 150px 1fr; gap: 20px; align-items: center;">
            <div class="form-check">
                <input type="checkbox" id="schedule_{{ $key }}" name="schedules[{{ $key }}][enabled]" value="1">
    ...
    <div style="display: flex; gap: 10px; margin-top: 30px;">
        <button type="submit" class="btn btn-primary">Salvează Doctorul</button>
