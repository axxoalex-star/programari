...
<div class="page-header">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
            <h1>Specialitate</h1>
            <p>Gestionează specializările clinicii</p>
        </div>
        <a href="{{ route('admin.departments.create') }}" class="btn btn-primary">+ Adaugă Specialitate</a>
    </div>
</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <form method="POST" action="{{ route('admin.departments.move-up', $department) }}" style="display:inline;">
                        @csrf
                        <button class="btn btn-sm btn-secondary" title="Mută sus">▲</button>
                    </form>
                    <form method="POST" action="{{ route('admin.departments.move-down', $department) }}" style="display:inline;">
                        @csrf
                        <button class="btn btn-sm btn-secondary" title="Mută jos">▼</button>
                    </form>
                    <a href="{{ route('admin.departments.edit', $department) }}" class="btn btn-sm btn-primary">Editează</a>
...
@else
    <div style="text-align: center; padding: 60px 20px; color: #999;">
        ...
        <a href="{{ route('admin.departments.create') }}" class="btn btn-primary" style="margin-top: 20px;">Creează prima specialitate</a>
    </div>
@endif
@endsection
